...
{
	std::normal_distribution<double> m_momentaDistribution(0.0, 1.0);

	for(auto& p : m_momenta)
	{
		p = m_momentaDistribution(generator);
	}
}
...
void HMCLattice1D::leapFrog(int lfStepCount, double lfStepSize, double alpha)
{
    ...
    for(int n = 0; n < lfStepCount/2; ++n)
    {
        ...
        {
            m_data[i] = m_data[i] - ((m_mass/m_spacing) * (2 * m_data[i] - m_data[i+1] - m_data[i-1] ) + m_spacing * (*m_potential)[m_data[i]]) * (lfStepSize/2.0);
        }
